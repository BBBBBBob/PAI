...
def log_posterior_probs(x):
    ...
    norm_dist = HYPOTHESIS_SPACE[0]
    p_norm = norm_dist.logpdf(x)
    laplace_dist = HYPOTHESIS_SPACE[1]
    p_laplace = laplace_dist.logpdf(x)
    t_dist = HYPOTHESIS_SPACE[2]
    p_t = t_dist.logpdf(x)
    ...
    zahler_norm = np.sum(p_norm) + np.log(h1)
    zahler_laplace = np.sum(p_laplace) + np.log(h2)
    zahler_t = np.sum(p_t) + np.log(h3)

    mutter = logsumexp(np.array([zahler_norm, zahler_laplace, zahler_t]))

    log_p = np.zeros((3, ))
    log_p[0] = zahler_norm - mutter
    log_p[1] = zahler_laplace - mutter
    log_p[2] = zahler_t - mutter
