...
class BO_algo(object):
    def __init__(self):
        ...
        self.objective_std = 0.01
        self.constraint_std = 0.005
        self.constraint_kernel = ConstantKernel(3.5) * RBF(length_scale=2)
        self.objective_model_kernel = ConstantKernel(1.5) * RBF(length_scale=2)
        self.constraint_model = GaussianProcessRegressor(kernel=self.constraint_kernel, alpha=self.objective_std**2)  # TODO : GP model for the constraint function
        self.objective_model = GaussianProcessRegressor(kernel=self.objective_model_kernel, alpha=self.constraint_std**2)  # TODO : GP model for your acquisition function
    ...
    def next_recommendation(self) -> np.ndarray:
        ...
        if len(self.previous_points) == 0:
            x0 = domain_x[:, 0] + (domain_x[:, 1] - domain_x[:, 0]) * np.random.rand(domain_x.shape[0])
            next_x = np.array([x0]).reshape(-1, domain_x.shape[0])
            # print("successfully return first recommend ")

        else:
            next_x = self.optimize_acquisition_function()
            # print("successfully run optimize_acquisition_function ")
    ...
    def acquisition_function(self, x: np.ndarray) -> np.ndarray:
        ...
        # TODO: enter your code here
        # Get f(x_best) from the previous points
        #print(np.shape(x),'x')
        x = x.reshape(-1, 2)  # (shoule be (1,2) instead of (2,))
        # print('acquisition calculate in ',x)
        f_value_list = self.previous_points_np[:, 2]  # value of the acquisition function at x
        f_best = np.min(f_value_list)
        mu_obj, sigma_obj = self.objective_model.predict(x, return_std=True)

        gamma = (f_best-mu_obj)/sigma_obj
        EI = float(sigma_obj*(gamma*norm.cdf(gamma)+norm.pdf(gamma)))

        # print("successifully compute EI_improvement",EI_improvement)

        # constraint function
        mu_c, sigma_c = self.constraint_model.predict(x, return_std=True)
        prob = norm.cdf(LAMBDA, loc=mu_c, scale=sigma_c)   # c(x) <= LAMBDA
        # print("successifully compute constraint prob",Pr_c)

        # Pr_C should be multi multiple
        # print('EI_improvement',np.shape(EI_improvement))
        # print('Pr_c',np.shape(-Pr_c))
    ...
    def add_data_point(self, x: np.ndarray, z: float, c: float):
        ...
        # TODO: enter your code here
        self.previous_points_np = np.array(self.previous_points)

        # print(self.previous_points_np[:,0:2],'self.previous_points[:,0:2]')
        # print(np.shape(self.previous_points_np[:,0:2]),'self.previous_points_np[:,0:2]')
        self.objective_model.fit(self.previous_points_np[:, 0:2], self.previous_points_np[:, 2])
        self.constraint_model.fit(self.previous_points_np[:, 0:2], self.previous_points_np[:, 3])
        #print("successifully add points",x)
    ...
    def get_solution(self) -> np.ndarray:
        ...
        constraint_feasible_sample = self.previous_points_np[:, 2]
        constraint_feasible_sample[self.previous_points_np[:, 3] > LAMBDA] = 1e6
        best_idx = np.argmin(constraint_feasible_sample)

        # TODO: enter your code here
        return self.previous_points_np[best_idx, 0:2]
...
CONSTRAINT_OFFSET = - 0.1  # This is an offset you can change to make the constraint more or less difficult to fulfill
