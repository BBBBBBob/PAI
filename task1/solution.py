...
class Model(object):
    ...
    def predict(self, x: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        gp_mean, gp_std = self.rbf_model.predict(x, return_std=True)
        predictions = gp_mean
    ...
    def fit_model(self, train_x: np.ndarray, train_y: np.ndarray):
        ...
        kernel = RBF(1, (10**-2, 10)) + ConstantKernel(0.001)
        self.rbf_model = GaussianProcessRegressor(kernel=kernel, random_state=1, n_restarts_optimizer=10, alpha=0.01)
        self.rbf_model.fit(train_x[:8000, :], train_y[:8000])
