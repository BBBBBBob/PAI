        ...
        for _ in progress_bar:
            ...
            for batch_idx, (batch_x, batch_y) in enumerate(train_loader):
                ...
                if isinstance(self.network, DenseNet):
                    ...
                    loss = F.cross_entropy(current_logits, batch_y, reduction='sum')
                    ...
                    log_likelihood = F.cross_entropy(current_logits, batch_y, reduction='sum')
                    # print('log', log_likelihood)
                    # print('nll', F.nll_loss(F.log_softmax(current_logits, dim=1), batch_y, reduction='sum'))
                    kl_weight = 0.002
                    loss = kl_weight*(log_variational_posterior - log_prior) + log_likelihood
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.prior = UnivariateGaussian(mu=torch.tensor(0.0), sigma=tensor(0.1))
        # {'prior_mu': torch.tensor(0.0), 'prior_sigma': tensor(1.0)}
        ...
        #  )
        self.weights_var_posterior = MultivariateDiagonalGaussian(mu=torch.nn.Parameter(torch.zeros(out_features, in_features).normal_(0, 0.5)),
                                                                  rho=torch.nn.Parameter(torch.zeros(out_features, in_features).normal_(-3, 1)))
        assert isinstance(self.weights_var_posterior, ParameterDistribution)
        assert any(True for _ in self.weights_var_posterior.parameters()), 'Weight posterior must have parameters'

        # Check
        # print(self.weights_var_posterior.mu.shape,'self.weights_var_posterior.mu.shape')
        # print('MultivariateDiagonalGaussian good')
        ...
        if self.use_bias:
            ...
            self.bias_var_posterior = MultivariateDiagonalGaussian(mu=torch.nn.Parameter(torch.zeros(out_features).normal_(0, 0.5)),
                                                                   rho=torch.nn.Parameter(torch.zeros(out_features).normal_(-3, 1)))
            # print('self.bias_var_posterior',self.bias_var_posterior.sample())
            #{'bias_mu': torch.nn.Parameter(torch.zeros((out_features))), 
            #' bias_sigma': torch.nn.Parameter(torch.ones((out_features)))}
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        # print('inputs',inputs.size()) ([128, 784])
        
        eps_weight = torch.rand_like(self.weights_var_posterior.mu)
        weights = self.weights_var_posterior.mu + F.softplus(self.weights_var_posterior.rho) * eps_weight
        log_prior_weight = self.prior.log_likelihood(weights)
        # print('weights',weights.size()) weights torch.Size([100, 784])

        log_variational_posterior_weight = self.weights_var_posterior.log_likelihood(weights)
        log_prior_bias = None
        log_variational_posterior_bias = None
        bias = None
        if self.use_bias:
            eps_bias = torch.rand_like(self.bias_var_posterior.mu)
            bias = self.bias_var_posterior.mu + F.softplus(self.bias_var_posterior.rho) * eps_bias
            log_prior_bias = self.prior.log_likelihood(bias)
            log_variational_posterior_bias = self.bias_var_posterior.log_likelihood(bias)

        log_variational_posterior = log_variational_posterior_weight + log_variational_posterior_bias
        log_prior = log_prior_weight + log_prior_bias
...
class BayesNet(nn.Module):
    ...
    def __init__(self, in_features: int, hidden_features: typing.Tuple[int, ...], out_features: int):
        ...
        # print('feature_sizes',feature_sizes) feature_sizes (784, 100, 100, 10)
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        current_features = x
        list_log_prior = torch.zeros(layers_length)
        list_log_variational_posterior = torch.zeros(layers_length)
        ...
        for idx, current_layer in enumerate(self.layers):
            ...
            # print('finish layer index',idx)
            if idx < len(self.layers) - 1:
                new_features = self.activation(new_features)
            current_features = new_features
            list_log_prior[idx] = new_log_prior
            list_log_variational_posterior[idx] = new_log_variational_posterior
                #log_prior = torch.cat((log_prior,new_log_prior),0) #  multiple backwards bug
                #log_variational_posterior=torch.cat((log_variational_posterior,new_log_variational_posterior),0)
        
        output_features = current_features
        log_prior = list_log_prior.sum()
        log_variational_posterior = list_log_variational_posterior.sum()
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def __init__(self, mu: torch.Tensor, rho: torch.Tensor):
        ...
        self.mu = mu
        self.rho = rho
        self.distribution = Normal(self.mu.data, F.softplus(self.rho))
